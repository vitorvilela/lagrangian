...
#define RANGE_ID NPV*2 (fator of 2 means 50% of probability to find a new id)
...
int main(int argc, char *argvp[]) {
  ...
  //    *****************  MOVE PARTICLE TO OTHER VOLUME  ***********************
    ...
    if(ptr_particle == NULL) 
      ...
      printf("\n\n\t\t*** GET PARTICLE (%i, %i) ***\n\n", ptr_volume->index.ix, ptr_particle->volume_id);
    ...
    tmp2 = malloc(sizeof(notional_particle_t));
        
    do {
      new_particle_id = rand()%RANGE_ID;
      HASH_FIND_INT(toVolume->particle, &new_particle_id, tmp2);
	...
	printf("\n\t\t\t    Particle (%i) - PDF(%f) \n\n", iter_particle->volume_id, iter_particle->pdf);
      }
    }
...
void ADD_PARTICLE(volume_t *ptr_volume, int id, double pdf) {
  ...
  particle->volume_id = id;
  particle->index.ix = ptr_volume->index.ix;
  particle->pdf = pdf;
  
  HASH_ADD_INT(ptr_volume->particle, volume_id, particle); 
  
  ptr_volume->last_used_id = ptr_volume->last_used_id + 1;
  
  printf("\n\t\t*** VOLUME (%i) -> ADD_PARTICLE(%i, %i) ***\n\n", ptr_volume->index.ix, particle->index.ix, particle->volume_id);
...
void COPY_PARTICLE(volume_t *ptr_volume, notional_particle_t *ptr_particle, int id) {
  ...
  printf("\n\n\t\t*** COPY PARTICLE (%i, %i) - PDF(%f) TO VOLUME (%i) WITH NEW ID (%i) ***\n\n", ptr_particle->index.ix, ptr_particle->volume_id, ptr_particle->pdf, particle->index.ix, particle->volume_id);
