...
int main(int argc, char *argvp[]) {
  ...
  for(t=1; t<=iterations; t++) {
	...
	for(ptr_particle=iter_head->sub; ptr_particle!=NULL; ptr_particle=(struct notional_particle*)(ptr_particle->hh.next)) {
	  
	  
	  
	  //ptr_particle = get_particle(iter_head, ptr_particle->id);
