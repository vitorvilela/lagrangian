...
void PRINT_VTK(notionalParticle_t *notionalParticleArray, double k) {
  ...
  HASH_ITER(hh, eulerian, iter_volume, tmp_volume) { 
    HASH_ITER(hh, iter_volume->id_np, iter_particle, tmp_particle) {   
      fprintf(pvtk, "%1.2e %1.2e %1.2e \n", notionalParticleArray[iter_particle->id].posix, notionalParticleArray[iter_particle->id].posiy, notionalParticleArray[iter_particle->id].posiz);
    }
  }
  ...
  HASH_ITER(hh, eulerian, iter_volume, tmp_volume) { 
    HASH_ITER(hh, iter_volume->id_np, iter_particle, tmp_particle) {     
      fprintf(pvtk, "%f \n", notionalParticleArray[iter_particle->id].zetap[0]);
    }
  }
