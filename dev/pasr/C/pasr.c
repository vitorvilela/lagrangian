...
#define tol_np 1.1
#define np_total np_vol_ini*nvol*tol_np  //Becarefull! it is a float type but it must be an integer
...
int main() {
  ...
  PDF_PRINT(notionalParticleArray, 0.0);
    ...
    while(stime < FINAL_TIME) {
      ...
      printf("\nstime = %f\n", stime);
//       gets(in);
      MOVE_MICROMIXING(notionalParticleArray, A, 1, ptr_idum3);
...
//       
      ...
      notionalParticleArray[part_counter].weight = (rho_cte*dx*dy*dz)/np_vol_ini;
...
void PDF_PRINT(notionalParticle_t *notionalParticleArray, double stime) {
  ...
  for(i=1; i<(n_interv+1); i++)     
	...
	sprintf(file_name, "PDF_I%iJ%iK%i(%1.2f).dat", iter_volume->index.igx, iter_volume->index.igy, iter_volume->index.igz, stime);  
